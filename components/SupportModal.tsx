...
import { CloseIcon, CoffeeIcon } from './icons';
import { useAppContext } from '../AppContext';
import { trackEvent } from './analytics';
import ActionButton from './ActionButton';

const SupportModal: React.FC = () => {
    const { setShowSupportModal, t } = useAppContext();

    const onClose = () => setShowSupportModal(false);

    const handleProceed = () => {
        trackEvent('support_modal_proceed', {});
        window.open('https://ko-fi.com/teant', '_blank', 'noopener,noreferrer');
        onClose();
    };
    ...
    return (
        ...
        >
            ...
            >
                ...
                <h2 id="support-modal-title" className="text-xl font-bold mb-3 text-stone-800">{t('supportModalTitle')}</h2>
                
                <p className="text-stone-600 mb-8">{t('supportModalMessage')}</p>
                ...
                <div className="flex flex-col sm:flex-row-reverse gap-3">
                    ...
                    >
                        <span>{t('supportModalProceed')}</span>
                    </ActionButton>
                     <ActionButton
                        onClick={onClose}
